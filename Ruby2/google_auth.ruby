...
def generate_user_credential(path)
  ...
  creds, callback_uri, port = if json["installed"]
  ...
  authorization_url = user_authorizer.get_authorization_url(state:)
...
def get_authorization_code(port)
  ...
  response_params = CGI.parse(URI.parse(matcher[1]).query) if matcher
